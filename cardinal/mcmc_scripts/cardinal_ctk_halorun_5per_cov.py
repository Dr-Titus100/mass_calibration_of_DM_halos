...
if __name__ == "__main__":
    ...
    # cov_path = "/global/homes/t/titus/Titus/lambda_cov/cluster-lensing-cov/examples/lam_cov_matrix_cardinal_sqrt84/"
    ...
    for i in range(len(halos_ds)):
        ...
        c = halos_ds["cvir"][i]
        true_params = [log10_M, c] #initial guess, starting point for MCMC walkers.
        id_cov = np.identity(len(rp[sel]))
        ds = log_likelihood(params = true_params, data = np.zeros(len(rp[sel])), cov = id_cov, z = z)[1]
        
        ######################
        cov_5per = np.diag((ds*0.05)**2)  # 5% uncertainty
        # cov = np.diag((ds1*0.10)**2)  # 10% uncertainty
        ...
        sampler = run_mcmc(data = ds, params = start, nwalkers = 32, nsteps = 10000, 
                           burnin = 1000, cov = cov_5per, z = z, readerfile = readerfile)

# mpirun -np 8 python cardinal_ctk_halorun_5per_cov.py --run_name _testrun_ctk_halorun_modelasdata_5per_cov
# mpirun -np 8 python cardinal_ctk_halorun_5per_cov.py --run_name _fullrun_ctk_halorun_modelasdata_5per_cov
