...
def log_likelihood(params, data, cov, z):
    ...
    large_scale_sel = large_scale_sel0[sel]
    # covariance
    icov_cut = np.linalg.inv(cov)
    
    icov_cut = icov_cut[sel][large_scale_sel]
    icov_cut = icov_cut[:,sel][:, large_scale_sel]
    
    # Difference between data and model and the likelihood.
    data_diff = (data - model)[large_scale_sel]
...
if __name__ == "__main__":
    ...
    # for i in range(len(halos_ds)-1, len(halos_ds)):
...
# mpirun -np 8 python cardinal_ctk_halorun_large_scale.py --run_name _testrun_ctk_halorun_large_scale
# mpirun -np 8 python cardinal_ctk_halorun_large_scale.py --run_name _fullrun_ctk_halorun_large_scale
# mpirun -np 8 python cardinal_ctk_halorun_large_scale.py --run_name _fullrun_ctk_halorun_large_scale_3_30_new
