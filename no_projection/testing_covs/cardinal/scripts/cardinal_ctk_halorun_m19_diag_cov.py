...
if __name__ == "__main__":
    ...
    # sel2 = halos_ds["Mvir"] > 14.0
    # halos_ds = halos_ds[sel2]
    ...
    for i in range(len(halos_ds)):
        ...
        ds = log_likelihood(params = true_params, data = np.zeros(len(rp[sel])), cov = cov_diag, z = z)[1]
        sampler = run_mcmc(data = ds, params = start, nwalkers = 32, nsteps = 10000, 
                           burnin = 1000, cov = cov_diag, z = z, readerfile = readerfile)

# mpirun -np 8 python cardinal_ctk_halorun_m19_diag_cov.py --run_name _testrun_ctk_halorun_modelasdata_m19_diag_cov
# mpirun -np 8 python cardinal_ctk_halorun_m19_diag_cov.py --run_name _fullrun_ctk_halorun_modelasdata_m19_diag_cov
