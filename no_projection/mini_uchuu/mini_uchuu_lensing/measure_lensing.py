...
class MeasureLensing(object):
    def __init__(self, out_loc, Rmin, Rmax, pimax): # pimax cannot be too large for TNG 
        ...
        nrp_per_decade = 5
        self.n_rp = int(nrp_per_decade*n_decade + 1 + 0.001) + 1 ## adding the inner most bin # (Titus) Also, I added +1 to self.n_rp to much McClintock radial bins
    ...
    def measure_lensing(self, xh, yh, zh, xp, yp, zp, boxsize, mpart):
        ...
        results_DD = countpairs_rp_pi(autocorr=0, nthreads=nthreads, pimax=self.pimax, binfile=self.binfile, 
            ...
            boxsize=(np.float64(boxsize), np.float64(boxsize), np.float64(boxsize)), output_rpavg=True, 
...
#     ouput_loc = '/global/u2/t/titus/Lensing/ouput/'
#     nbody_loc = '/global/u2/t/titus/Lensing/data/'
