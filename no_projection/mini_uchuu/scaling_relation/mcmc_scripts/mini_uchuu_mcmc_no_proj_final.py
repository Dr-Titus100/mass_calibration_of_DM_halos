...
def log_likelihood(params, DS_data, sac, boost_data, z, lam, boost_cov):
    ...
    # lnrp_bins = np.linspace(np.log(Rmin), np.log(Rmax), nbins+1)
    # rp_bins = np.exp(lnrp_bins)
...
if __name__ == "__main__":
    ...
    # rp_, Sigma0, DeltaSigma0 = ml.measure_lensing(xh[sel][sel0], yh[sel][sel0], zh[sel][sel0], xp, yp, zp, boxsize, mpart)
    # rp_, Sigma1, DeltaSigma1 = ml.measure_lensing(xh[sel][sel1], yh[sel][sel1], zh[sel][sel1], xp, yp, zp, boxsize, mpart)
    # rp_, Sigma2, DeltaSigma2 = ml.measure_lensing(xh[sel][sel2], yh[sel][sel2], zh[sel][sel2], xp, yp, zp, boxsize, mpart)
    # rp_, Sigma3, DeltaSigma3 = ml.measure_lensing(xh[sel][sel3], yh[sel][sel3], zh[sel][sel3], xp, yp, zp, boxsize, mpart)
    ...
    parser.add_argument('--l_scale_cut', type=float, default=0.2, help='Lower scale cut value in floating point')
    ...
    for j in range(start,end):
        ...
        # Rb = np.loadtxt(lensing_loc+"full-unblind-v2-mcal-zmix_y1clust_l"+str(j+3)+"_z"+str(i)+"_zpdf_boost.dat")[:,0] #Boost radii        
        ...
        ##################################################################
        ##################################################################
        ### Creating and saving DS and boost data
        # # # ds and boost profiles
    ...
    # np.save(saved_boost_profiles_dic, boost_dic)
        ...
        # boost_data = ctk.boostfactors.boost_nfw_at_R(rp, B0_orig, Rs_orig*h*(1+z))
    ...
    # mpirun -np 8 python mini_uchuu_mcmc_no_proj_final.py --redshift 0 --start 0 --end 4 --sys_name _phys_units_all_scale_final_final
    
    # Added the factor of h*(1+z) to all Rs in boost data, boost cov, Redges, except boost model, radial cut at 0.3Mpc to 3Mpc.
    # mpirun -np 8 python mini_uchuu_mcmc_no_proj_final.py --redshift 0 --start 0 --end 4 --l_scale_cut 0.2 --u_scale_cut 3.0 --sys_name _phys_units_small_scale_0.2_3_final_final
    
    # Added the factor of h*(1+z) to all Rs in boost data, boost cov, Redges, except boost model, radial cut at 3Mpc to 30Mpc.
    # mpirun -np 8 python mini_uchuu_mcmc_no_proj_final.py --redshift 0 --start 0 --end 4 --l_scale_cut 3.0 --u_scale_cut 999.0 --sys_name _phys_units_large_scale_3_30_final_final
