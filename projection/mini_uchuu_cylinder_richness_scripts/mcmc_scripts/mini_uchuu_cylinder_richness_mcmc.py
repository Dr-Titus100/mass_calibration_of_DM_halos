...
def log_flat_prior(args):
    ...
    if 11.0<log10_M<18.0 and 0.0<c<20.0 and 0.0<B0<5.0 and 0.0<Rs<5.0: #change from mvir to log_mvir
        return 0.0
    return -np.inf
...
def getargs(lam_bin, z_bin):
    # Values taken from the posterior means of our MCMC.
    ...
    # Concentrations
    c_dic = {"l3_z0":5.7876 , "l4_z0":5.4306, "l5_z0":5.03998, "l6_z0":5.6915}

    # Boost factor amplitude
    B_dic = {"l3_z0":0.3441, "l4_z0":0.3724, "l5_z0":0.2762, "l6_z0":0.2338}
    
    # sigma for boost factor amplitude from M19
    sigB_dic = {"l3_z0":0.05, "l4_z0":0.06, "l5_z0":0.05, "l6_z0":0.03}

    # Boost factor scale radius
    Rs_dic = {"l3_z0":0.4438, "l4_z0":0.5065, "l5_z0":0.8026, "l6_z0":1.3621}
    
    # sigma for boost factor scale radius from M19
    sigRs_dic = {"l3_z0":0.06, "l4_z0":0.07, "l5_z0":0.15, "l6_z0":0.21}
    
    # miscentering offset
    tau_dic = {"l3_z0":0.1857, "l4_z0":0.1818, "l5_z0":0.1770, "l6_z0":0.1784}
    
    # miscentering fraction
    fmis_dic = {"l3_z0":0.2566, "l4_z0":0.2579, "l5_z0":0.2560, "l6_z0":0.2731}
    ...
    Am_dic = {"l3_z0":1.0210, "l4_z0":1.0214, "l5_z0":1.0206, "l6_z0":1.0210}
    ...
    return mass_dic[lam_z_bin], c_dic[lam_z_bin], B_dic[lam_z_bin], Rs_dic[lam_z_bin], z_dic[str(z_bin)], Am_dic[lam_z_bin], Sigma_crit_dic[lam_z_bin], tau_dic[lam_z_bin], fmis_dic[lam_z_bin], sigRs_dic[lam_z_bin], sigB_dic[lam_z_bin], lam_z_bin
...
if __name__ == "__main__":
    ...
    parser.add_argument('--l_scale_cut', type=float, default=0.01, help='lower scale cut value in floating point')
    parser.add_argument('--u_scale_cut', type=float, default=999.0, help='upper scale cut value in floating point')
    ...
    data_ds = np.load(data_path+f'mini_uchuu_ds_cylinder_richness_selection_d{cyl_depth}_no_pecvel_changing_vol_heidi_with_sys_final.npy', allow_pickle=True).item()
    
    # Mean mass of each bin
    mass = np.load(data_path+f'mini_uchuu_mass_cylinder_richness_selection_d{cyl_depth}_no_pecvel_changing_vol_heidi_final.npy', allow_pickle=True).item()
    ...
    for j in range(start,end):
        ...
        if j+3 == 3:
            low = 20
            high = 30
            mass_orig, c_orig, B0_orig, Rs_orig, z, Am_orig, Sigma_crit_inv0, tau_orig, fmis_orig, sigRs, sigB0, lam_z_bin = getargs(j+3,i)
            # mass, concentration, B0, Rs, tau (miscentering offset), fmis, Am.
            true_params = np.array([np.log10(mass_orig), c_orig, B0_orig, Rs_orig, tau_orig, fmis_orig, Am_orig])  
        ...
        ds = data_ds["l"+str(j+3)+"_z"+str(i)] # DeltaSigma profile
        ...
        start_params = np.array([14.20, 4.20, 0.3, 0.7, 0.17, 0.25, 1.0]) # mass, concentration, B0, Rs, tau (miscentering offset), fmis, Am.
        nsteps = 10000
