...
class DrawSatPosition(object):
    def __init__(self, yml_fname, seed = None):
        ...
        if self.mdef == 'vir':
            mass_interp = 10**np.linspace(10, 16, 50)
            c_interp = concentration.concentration(mass_interp, z=self.redshift, mdef=self.mdef, model='ishiyama21')
        else:
            mass_interp = 10**np.linspace(11, 16, 20) # Heidi's original
    ...
    def draw_sat_velocity(self, mass, Nsat): # Evrard 2008
        ...
        c = self.c_lnM_interp(np.log(mass))
        M200c, R200c, c200c = mass_defs.changeMassDefinition(mass, c, self.redshift, self.mdef, '200c')
        OmegaM = self.Om0
