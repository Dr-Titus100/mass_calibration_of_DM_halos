...
def log_likelihood(params, data, cov, z):
    ...
    all_scale_sel0 = (rp>=0.2)&(rp<999)
    all_scale_sel = all_scale_sel0[sel]
    # covariance
    icov_cut = np.linalg.inv(cov)
    
    icov_cut = icov_cut[sel][all_scale_sel]
    icov_cut = icov_cut[:,sel][:, all_scale_sel]
    
    # Difference between data and model and the likelihood.
    data_diff = (data - model)[all_scale_sel]
...
if __name__ == "__main__":
    ...
    for i in range(len(halos_ds)-2, len(halos_ds)):
