...
def log_likelihood(params, data, cov, z):
    ...
    mid_scale_sel0 = (rp>=0.2)&(rp<3)
    mid_scale_sel = mid_scale_sel0[sel]
    # covariance
    icov_cut = np.linalg.inv(cov)
    
    icov_cut = icov_cut[sel][mid_scale_sel]
    icov_cut = icov_cut[:,sel][:, mid_scale_sel]
    
    # Difference between data and model and the likelihood.
    data_diff = (data - model)[mid_scale_sel]
...
if __name__ == "__main__":
    ...
    for i in range(len(halos_ds)-3, len(halos_ds)):
        ...
        ds = halos_ds["DS_mean"][i][sel]#*1e-12

        #######################
        c = halos_ds["cvir"][i]
        args = [14.20, 3.9] #initial guess, starting point for MCMC walkers.
        sampler = run_mcmc(data = ds, params = args, nwalkers = 32, nsteps = 10000, 
                           burnin = 1000, cov = cov, z = z, readerfile = readerfile)

# mpirun -np 8 python cardinal_ctk_halorun_mid_scale.py --run_name _testrun_ctk_halorun_mid_scale
# mpirun -np 8 python cardinal_ctk_halorun_mid_scale.py --run_name _fullrun_ctk_halorun_mid_scale
# mpirun -np 8 python cardinal_ctk_halorun_small_scale.py --run_name _fullrun_ctk_halorun_small_scale_0.2_3_new_new
